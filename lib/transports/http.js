...
class HTTPTransport {
  ...
  buildRequest (link, decoders, params = {}, extraParams = {}) {
    ...
    for (var extraProperty in extraParams) {
      if (method === 'GET') {
        queryParams[extraProperty] = extraParams[extraProperty]
      } else {
        formParams[extraProperty] = extraParams[extraProperty]
        hasBody = true
      }
    }
  ...
  action (link, decoders, params = {}, extraParams = {}) {
    const responseCallback = this.responseCallback
    const request = this.buildRequest(link, decoders, params, extraParams)
